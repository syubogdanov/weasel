...
class SQLMutation(MutationInterface):
    """The *SQL* mutation (`SQL005`).

    Features
    --------
    * Each expression starts from a new line;
    * Convert some predicates with subqueries into joins.
    """
    ...
    def _mutate(cls, source: str) -> str:
        ...
        as_strings = [
            optimize(expression, rules=[unnest_subqueries]).sql() for expression in expressions
        ]
        return ";\n".join(as_strings) + ";"
