...
class ReportEntity(BaseModel):
    ...
    def to_json(self, path: Path, *, indent: int | None = None, encoding: str = "utf-8") -> None:
    ...
    def to_yaml(self, path: Path, *, indent: int | None = None, encoding: str = "utf-8") -> None:
        """Dump the entity as *YAML*."""
        data = self.model_dump(mode="json")
        with path.open(mode="w", encoding=encoding) as file:
            yaml.safe_dump(data, file, default_flow_style=False, indent=indent, sort_keys=True)
